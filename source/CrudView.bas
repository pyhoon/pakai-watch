...
Private Sub ItemsTableFilled (data As List) As MiniHtml
	...
	For Each row As Map In data
		...
		tr1.ChildByIndex(1).text2(row.Get("item_code"))
		tr1.ChildByIndex(2).text2(row.Get("item_name"))
		tr1.ChildByIndex(3).text2(NumberFormat2(row.Get("item_price"), 1, 2, 2, True))
		tr1.ChildByIndex(4).text2(row.GetDefault("item_description", ""))
